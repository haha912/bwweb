				...
				submitHandler: function(form) {
					...
					$.ajax({
						...
						data: {
							...
							verifyCode: $("#contact-form #verifyCode").val()
						},
						dataType: "json",
						success: function (data) {
							if (data.sent == "yes") {
								...
								$("#captcha-error").css("visibility","hidden");
								$(".submit-button").removeClass("btn-default").addClass("btn-success").prop('value', 'Message Sent');
								$("#contact-form .form-control").each(function() {
									$(this).prop('value', '').parent().removeClass("has-success").removeClass("has-error");
								});
							} else if(data.sent == "coderror"){
								$(".capt").addClass("has-error");
		...
		if($("#sidebar-form").length>0) {
			...
			$("#sidebar-form").validate({
				submitHandler: function(form) {
					$('.submit-button').button("loading");
					$.ajax({						
						url: "/home/faq/addquestion",
						type: "POST",
 						data: {
							name: $("#sidebar-form #name3").val(),
							email: $("#sidebar-form #email3").val(),
							subject: $("#sidebar-form #category").val(),
							message: $("#sidebar-form #message3").val(),
							verifyCode: $("#sidebar-form #verifyCode").val()
						}, 
						dataType: "json",
						success: function (data) {
							if (data.sent == "yes") {								
								$("#MessageSent3").removeClass("hidden");
								$("#another-ques").removeClass("hidden");
								$("#MessageNotSent3").addClass("hidden");
								$("#captcha-error").css("visibility","hidden");
								$(".form-group").addClass("hidden");
								$(".submit-button").removeClass("btn-default").addClass("btn-success").prop('value', '问题已发送');
...
/* 					var url= "/faq/addquestion";  
		...
		if($("#contact-form-with-recaptcha").length>0) {
			$("#contact-form-with-recaptcha").validate({
				submitHandler: function(form) {
					...
					$.ajax({
						type: "POST",
						url: "/static/php/email-sender-recaptcha.php",
						data: {
		...
		if($("#inline-form").length>0) {
			...
			$("#inline-form").validate({
				submitHandler: function(form) {
					$('#sub-utton').button("loading");
					console.log($("#inline-form #subscribe").val());
					$.ajax({						
						url: "/home/contact/addsubscribe",
						type: "POST",
 						data: {
							email: $("#inline-form #subscribe").val()
						}, 
						dataType: "json",
						success: function (data) {
							if (data.sent == "yes") {				
								$("#sub-utton").removeClass("btn-default").addClass("btn-success").prop('value', '已订阅');
							}
						}
					});
/* 					var url= "/faq/addquestion";  
